...
class PhoneVerificationController extends Controller
{
    ...
    public function resend_otp_check_phone(Request $request){
        ...
        $otp_resend_time = Helpers::get_business_settings('otp_resend_time') > 0 ? Helpers::get_business_settings('otp_resend_time') : 0;
        $user = User::where(['temporary_token' => $request->temporary_token])->first();
        $token = PhoneOrEmailVerification::where('phone_or_email',isset($user['phone']) ? $user['phone'] : null)->first();
        ...
        if($time_differance==0){
            ...
            if($token){
                ...
                $token_data->phone_or_email = isset($user['phone']) ? $user['phone'] : null;
            ...
            if($published_status == 1){
                $response = SmsGateway::send(isset($user['phone']) ? $user['phone'] : null, $new_token);
            }else{
                $response = SMS_module::send(isset($user['phone']) ? $user['phone'] : null, $new_token);
            }

            return response()->json([
                'message' => $response == 'not_found' ? translate('otp_send_failed'):translate('sent_otp'),
                'token' => 'active',
                'new_time'=> $otp_resend_time,
            ], $response == 'not_found' ? 403 : 200);
        } else {
            return response()->json([
    ...
    public function verify_phone(Request $request)
    {
        ...
        $temp_block_time = Helpers::get_business_settings('temporary_block_time') ?? 5; //minute
        $user = User::where(['temporary_token' => $request['temporary_token']])->first();
        $verify = PhoneOrEmailVerification::where(['phone_or_email' => isset($user['phone']) ? $user['phone'] : null, 'token' => $request['otp']])->first();
        ...
        if (isset($verify)) {
            ...
            $user->phone = isset($user['phone']) ? $user['phone'] : null;
            ...
            $verification = PhoneOrEmailVerification::where(['phone_or_email' => isset($user['phone']) ? $user['phone'] : null])->first();
